    ...
    <div className={`min-h-screen flex flex-col relative z-10 overflow-hidden ${isKidMode ? 'kid-grid text-slate-800' : 'scholar-grid bg-slate-950 text-slate-100'}`}>
      ...
      <main className="flex-1 w-full max-w-6xl mx-auto px-3 sm:px-6 py-8 relative z-10">
        ...
        <div className="w-full text-left mb-8">
          ...
          <p className={`text-sm mt-1.5 leading-relaxed ${isKidMode ? 'text-slate-655 font-medium' : 'text-slate-400 font-mono text-[11px]'}`}>
            {isKidMode 
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-8 w-full">
          ...
          <div className="flex-1 relative">
            ...
            <input 
              ...
              className={`w-full pl-11 pr-4 py-3 rounded-2xl text-xs sm:text-sm transition-all focus:outline-none ${
                isKidMode 
                  ? 'border-4 border-slate-800 text-slate-800 bg-white placeholder-slate-450 focus:shadow-[2px_2px_0_#1E293B]' 
          ...
          <div className="flex gap-2">
            <div className={`p-1.5 rounded-2xl flex items-center gap-1.5 border ${
              isKidMode ? 'bg-white border-slate-800' : 'bg-slate-900/60 border-slate-850'
            }`}>
              ...
              >
                <option value="all" className={isKidMode ? "text-slate-805" : "bg-slate-950 text-slate-200"}>
                  {isKidMode ? 'Semua Kelangkaan' : 'All Rarities'}
                </option>
                <option value="common" className={isKidMode ? "text-slate-805" : "bg-slate-950 text-slate-200"}>Common</option>
                <option value="rare" className={isKidMode ? "text-slate-805" : "bg-slate-950 text-slate-200"}>Rare</option>
                <option value="epic" className={isKidMode ? "text-slate-805" : "bg-slate-950 text-slate-200"}>Epic</option>
                <option value="legendary" className={isKidMode ? "text-slate-805" : "bg-slate-950 text-slate-200"}>Legendary</option>
                <option value="mythic" className={isKidMode ? "text-slate-805" : "bg-slate-950 text-slate-200"}>Mythic</option>
              </select>
            </div>
          </div>
        </div>
          ...
          <button
            ...
            className={`touch-target px-4 py-2.5 rounded-2xl text-xs font-black border-2 transition-all cursor-pointer flex-shrink-0 ${
              selectedCategory === 'all'
                ...
                : isKidMode
                  ? 'bg-white border-slate-850 text-slate-700 hover:bg-slate-50'
                  : 'bg-slate-900 border-slate-850 text-slate-450 hover:text-slate-200 hover:border-slate-755'
            }`}
          >
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between mb-4">
                        <div className={`w-12 h-12 rounded-full border-2 border-slate-800 flex items-center justify-center ${
                          isUnlocked 
                            ? isKidMode ? 'bg-pink-500' : 'bg-slate-850'
                            : 'bg-slate-200 dark:bg-slate-900 text-slate-400'
                        }`}>
                          {isSecret ? (
                            <Lock className="w-5 h-5 text-slate-550" />
                          ) : (
                      ...
                      <div className="mt-4 pt-3 border-t border-slate-800/10 dark:border-slate-800/40">
                        <div className="flex items-center justify-between text-[9px] font-bold text-slate-450 uppercase mb-1 font-mono">
                          <span>Progress</span>
        ...
        return (
          <div className="fixed inset-0 bg-black/85 backdrop-blur-sm z-55 flex items-center justify-center p-4">
            <div className={`w-full max-w-sm p-6 relative overflow-hidden modal-responsive transition-all duration-300 ${
              isKidMode 
                ? 'card-toy bg-gradient-to-b from-white to-pink-50 border-4 border-slate-800 shadow-[8px_8px_0_#1E293B] text-slate-805' 
                : 'glass-panel border border-slate-850 rounded-[32px] text-left bg-slate-900/95 shadow-2xl text-slate-200'
            }`}>
              ...
              <div className="text-center mb-5">
                <div className="flex justify-center mb-3">
                  <div className={`w-20 h-20 rounded-full border-4 border-slate-800 flex items-center justify-center relative shadow-md ${
                    ...
                    {isSecret ? (
                      <Lock className="w-8 h-8 text-slate-550" />
                    ) : (
                ...
                <span className={`text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded border ${
                  isKidMode ? 'bg-slate-100 border-slate-800 text-slate-805' : 'bg-slate-950 border-slate-800 text-slate-400 font-mono'
                }`}>
                  {selectedBadge.rarity}
                </span>

                <h3 className={`text-xl sm:text-2xl font-black mt-2.5 ${isKidMode ? 'text-slate-805 font-fredoka' : 'text-white'}`}>
              ...
              <p className={`text-xs leading-relaxed mb-5 ${isKidMode ? 'text-slate-655' : 'text-slate-400'}`}>
                {isSecret 
                    ...
                    <button
                      ...
                      className={`w-full py-2.5 text-center font-black text-xs uppercase font-mono tracking-widest cursor-pointer border-2 rounded-xl transition-all ${
                        isFavBadge
                          ...
                          : isKidMode
                            ? 'bg-white border-slate-850 text-slate-805 shadow-[2px_2px_0_#1E293B] font-bold'
                        ...
                        className={`w-full py-2.5 text-center font-black text-xs uppercase font-mono tracking-widest cursor-pointer border-2 rounded-xl transition-all ${
                          isFavTitle
                            ? isKidMode 
                              ...
                              : 'bg-rose-955/20 border-rose-500 text-rose-400'
                            : isKidMode
                              ? 'bg-white border-slate-855 text-slate-805 shadow-[2px_2px_0_#1E293B] font-bold'
                              : 'bg-slate-900 border-slate-800 text-slate-200 hover:border-slate-700'
                        }`}
                      >
